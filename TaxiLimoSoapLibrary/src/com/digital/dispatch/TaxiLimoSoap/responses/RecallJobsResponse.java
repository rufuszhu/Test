		...
		try {
			...
			for (int i = 0; i < properties.size(); i ++) {
				...
				if ((item instanceof SoapObject) && ( objName.equalsIgnoreCase("listOfJobs"))) {
					...
					JobItem ji = new JobItem(pName, phoneNum, phoneExt, pStreetNum, pStreetName, pRegion, pUnitNum,
							pLandmark, dStreetNum, dStreetName,	dRegion, dUnitNum, dLandmark, aCode, aName, pDate, 
							pTime, repPTime, repPDays, repSt, repEn, trID, bDateTime, tStatus, aArrival, dCar, 
							dDriver, cLongitude, cLatitude, tStatCode, dStatCode, e1, e2, eDis, jobEndDateTime);
					listOfJobs[j] = ji;
					j++;
				}
			}
